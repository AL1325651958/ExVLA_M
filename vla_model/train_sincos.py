"""Train ExcavatorVLA — sin/cos joint encoding variant.

qpos is encoded as [sin(theta), cos(theta)] to eliminate the 2pi
discontinuity in angular space. All other qpos logic (modulation,
dropout mask, training-only) is unchanged.

Inference: pure vision, zero qpos dependency.
Output: absolute joint angles.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train ExcavatorVLA — sin/cos joint encoding")
    ...
    parser.add_argument("--sample_ratio", type=float, default=None,
                        help="Fraction of data to use (0.2=20%% for fast training)")
    ...
    config.output_dir = "output/checkpoints_sincos"

    config.device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Device: {config.device}")
    print(f"Output dir: {config.output_dir}")
    print(f"Joint encoding: sin/cos (circular)")
    ...
    # Model — sin/cos joint encoding enabled
    model = ExcavatorVLA(
