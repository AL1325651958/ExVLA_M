"""Training script for ExcavatorVLA — Mamba encoder, 24-frame input.

Mamba: linear-time state-space encoder (O(n)), no attention.
qpos: head-level modulation + dropout mask (training only).
Output: absolute joint angles. Inference: pure vision.
24-frame input for longer temporal context.
"""
...
def main():
    ...
    parser.add_argument("--seq_len", type=int, default=24,
                        help="Sequence length (Mamba O(n), 24 frames default)")
