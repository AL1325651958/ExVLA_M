"""Excavator-STVTA V13 training entrypoint.

V13: V12 + Joint Relation Transformer + velocity auxiliary loss (10% chance of suppressing one branch).
"""
...
Config.v13_vel_aux_weight = 0.02
...
def _stvta_losses(model, raw_out, masks_spatial, pose_aux, action_gt, qpos, criterion, aux_weight, vel_aux=None, vel_weight=0.0):
    ...
    total_extra = aux_weight * pose_aux_loss
    if vel_aux is not None:
        vel_target = action_gt.squeeze(1)  # Δqpos target
        vel_aux_loss = criterion(vel_aux, vel_target)
        total_extra = total_extra + vel_weight * vel_aux_loss
    return (pred_loss + circle_loss + area_loss + temporal_loss +
            total_extra), action_gt_rad, {
...
def train_epoch(model, dataloader, optimizer, scheduler, scaler, criterion, config, epoch):
    ...
    for step, batch in enumerate(pbar):
        ...
        with autocast(enabled=config.device == "cuda"):
            outputs = model(rgb, elevation, excavator_id=excavator_id, return_aux=True)
            raw_out, _, masks_spatial, _, pose_aux, vel_aux = outputs
            vel_weight = getattr(config, "v13_vel_aux_weight", 0.02)
            loss, action_gt_rad, mask_stats = _stvta_losses(
                model, raw_out, masks_spatial, pose_aux, action_gt, qpos, criterion, aux_weight,
                vel_aux=vel_aux, vel_weight=vel_weight,
            )
