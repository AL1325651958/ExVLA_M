...
VLM 不输出连续关节角度、不直接向执行器发控制命令。它以低频闭环方式选择和切换低层技能，并根据新图像判断当前阶段是否完成、是否需要重规划或停止。每个低层策略保持纯视觉输入；V11 的模型前向不接收或使用 qpos。第一版候选模型为 Qwen2.5-VL-3B-Instruct，后续可在同一 JSON 协议下替换。

## L/VLM 的控制参与方式

第一版采用技能选择而非目标条件控制。VLM 的 `phase` 经校验后由调度器映射到一个阶段专用低层视觉策略：`lower`、`dig`、`lift`、`swing_to_dump`、`dump`、`return` 或 `stop`。低层策略在本阶段内连续输出下一时刻四关节角，VLM 不参与逐帧数值回归。

执行循环如下：

1. VLM 接收用户指令、关键 RGB/高程帧和上一轮执行反馈，输出计划 JSON。
2. 校验器验证 JSON schema、允许的阶段转移和安全约束；失败时强制 `stop` 或请求重规划。
3. 调度器选择与 `phase` 对应的低层纯视觉技能策略。
4. 低层策略执行一个受限短时间窗；安全层限制关节范围、速度和风险动作。
5. VLM 读取更新后的视觉观测，确认成功条件，决定保持当前技能、切换下一技能、重规划或停止。

因此，L/VLM 的输出会直接改变“哪个低层控制技能正在运行”，并通过阶段完成判定构成闭环；它不会越过安全层直接控制电机或关节。
...
第一阶段阶段集合固定为：`observe`、`approach`、`lower`、`dig`、`lift`、`swing_to_dump`、`dump`、`return`、`stop`。其中除 `observe` 和 `stop` 外的阶段均可映射到同名低层技能。目标区域以相机视角的 `front_left`、`front`、`front_right`、`left`、`right`、`unknown` 表示。安全校验器将在后续 VLM 阶段验证 schema、允许的阶段转移、置信度阈值与风险约束。
...
为每个候选片段导出预览视频，包含 RGB、高程图、四关节曲线、候选阶段和时间范围。审核者确认或修改阶段、目标区域、约束、下一阶段和完成条件。确认后的样本标记为 `source: human_verified`，并导出 JSONL；阶段标签同时作为后续训练阶段专用低层技能策略的切片依据。
...
- 第一阶段不微调 VLM、不接入真实执行器、不做边缘部署，也不训练阶段专用低层策略。
