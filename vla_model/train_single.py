"""Train ExcavatorVLA on a SINGLE excavator — no cross-machine interference.

Usage:
  python vla_model/train_single.py --excavator 75  # only 75 data
  python vla_model/train_single.py --excavator 306
  python vla_model/train_single.py --excavator 490

The model uses num_excavators=1 — one head, one machine, zero confusion.
"""
...
def train_epoch(model, dataloader, optimizer, scaler, scheduler, criterion, config, epoch):
    model.train()
    total_loss = 0.0; total_mae = np.zeros(4)
    ss_res = np.zeros(4); sum_y = np.zeros(4); sum_y2 = np.zeros(4)
    n_total = 0; n_batches = 0
    ...
    return {
        ...
        "r2": r2_per.tolist(), "r2_mean": r2_mean,
    }
...
def main():
    parser = argparse.ArgumentParser(description="Train ExcavatorVLA — single excavator")
    parser.add_argument("--excavator", type=str, required=True,
                        choices=["75", "306", "490"],
                        help="Which excavator to train on (single machine)")
    ...
    config = Config()
    # Point data_dir at the single excavator's folder
    config.data_dir = f"data/excavator-motion/data/{excv_name}"
    ...
    # Datasets — load from single-excavator folder
    train_dataset = ExcavatorDataset(
        ...
        force_excv_id=0,                 # single head → all IDs map to 0
    )
    ...
    # Model — single head (num_excavators=1)
    model = ExcavatorVLA(
        seq_len=config.seq_len, hidden_dim=config.hidden_dim,
