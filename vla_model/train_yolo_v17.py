"""V17 training: 4 independent mask heads + graded per-joint decoder gating.

V17 architecture features:
- 4 independent mask heads (one per joint) — no weight sharing
- Graded per-joint decoder: Boom(λ_m=1.5/λ_v=0.8), Arm(2.0/1.0), Bucket(2.5/1.0), Swing(0.0/0.5)
- All joints use residual value gating V*(1+λ_v*M) — never hard-gate with V*M
- Dual-modality 8-mask + cross-modal attention (inherited from V16)
- Swing: no logit bias, weak residual only — reads full global memory
- No encoder gating — mask filtering in decoder only

Checkpoints include "model_version": "v17".
"""
...
def train_epoch(model, dataloader, optimizer, scaler, scheduler, criterion, config, epoch):
    ...
    for step, batch in enumerate(pbar):
        ...
        with torch.amp.autocast(config.device):
            ...
            # V17: masks_spatial = [B, 2, 4, T, G, G] (modality × joint × time × spatial)
            ...
            # V17: [B, 2, 4, T, G, G] → flatten to [B, 8, T*G*G] for all-mask diversity
            # Use .mean() not .sum() — scale-invariant to number of masks (8 → 56 pairs).
...
def save_checkpoint(model, optimizer, scaler, scheduler, epoch, metrics, config, is_best=False):
    ...
    path = os.path.join(config.output_dir, f"yolo_v17_checkpoint_{suffix}.pt")
    torch.save({
        "epoch": epoch + 1,
...
def main():
    parser = argparse.ArgumentParser(description="Train ExcavatorVLA-YOLO V17")
    ...
    with open(os.path.join(config.output_dir, "yolo_v17_history.json"), "w") as f:
