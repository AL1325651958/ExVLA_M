...
## V8 — Joint Embedding Mask + Elevation Adapter + All sin/cos

- Shared mask_generator (scheme B): tokens + joint_embed[j] → mask_j, 证明 mask 差异来自条件
- 删除 qpos modulation: 训练=推理一致
- Elevation adapter: Conv(3→16)→BN→SiLU→Conv(16→3)
- 全 sin/cos 输出 [B,8]，Arm 从标量改为 sin/cos 消除抖动

---

## V9 — Mask-Biased Cross-Attention + Separate Necks

- **MaskBiasedCrossAttn**: `logits += λ·log(M+ε)`, `V *= M` → 真正的唯一入口
- 删除 encoder 因果掩码 (bidirectional history)
- 共享 mask_generator 保留
- 均方重叠² → **余弦相似度 margin**: `relu(cos_sim − 0.3)²`
- L1 稀疏 → **区间面积约束**: `[0.05, 0.30]`
- 单位圆归一化 + `0.1×(s²+c²−1)²` loss
- `rgb_neck` + `elev_neck` 独立 FPN-PAN (避免 BN 污染)
- `train_split=0.75` (3:1 train/val, 20/9 episodes)

**参数量**: 43.1M

---

## V10 — Temporal Mask Mixer + Vision-Only QPos Auxiliary

- **TemporalMaskMixer**: Transformer 交叉帧混合 [B,G²,T,D]，掩膜随运动演化
- **Pure visual inference**: `forward()` 不使用 qpos; `return_aux=True` 时才输出 `pose_aux [B,4]`
- **训练辅助**: `train_yolo_v10.py`, `pose_aux_weight=0.05`, MSE(pose_aux, qpos[:,-1])
- 可视化: V10 默认 `--mask_view last` (原始最后一帧), V9 默认 `avg`
- 向后兼容: 所有 V9 checkpoint 仍可加载
