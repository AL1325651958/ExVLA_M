"""Training script for ExcavatorVLA — modulation + delta prediction.

qpos: head-level modulation with dropout mask (training only).
Output: delta (change from current qpos), not absolute.
Inference: last_qpos + delta → absolute.

Loss: MSE(delta_pred, delta_label)
MAE/R²: computed on absolute values for fair comparison.
"""
...
def train_epoch(model, dataloader, optimizer, scaler, scheduler, criterion, config, epoch):
    ...
    for step, batch in enumerate(pbar):
        ...
        with autocast():
            delta_pred = model(rgb, elevation, qpos, excavator_id)   # [B, 4] delta
            action_label = action_gt.squeeze(1)                       # [B, 4] absolute next_qpos
            last_qpos = qpos[:, -1, :]                                # [B, 4]
            delta_label = action_label - last_qpos                    # [B, 4] delta
        ...
        # Convert to absolute for fair MAE/R² comparison
        abs_pred = (last_qpos + delta_pred.detach()).cpu().numpy()
        abs_label = action_label.cpu().numpy()
        mae = np.abs(abs_pred - abs_label).mean(axis=0)
        total_mae += mae

        # R² stats on absolute values
...
def main():
    ...
    # Separate output dir to avoid conflict with absolute model
    config.output_dir = "output/checkpoints_delta"
