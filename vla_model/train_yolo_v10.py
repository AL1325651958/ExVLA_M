"""V10 training: adds training-only qpos auxiliary supervision.

Same as train_yolo.py but:
- Model version="v10" with temporal mask mixer
- Training: model(..., return_aux=True) → additional pose_aux [B,4]
- pose_aux_loss = MSE(pose_aux, qpos[:,-1]) with weight 0.05
- Validation: normal 3-output path (no auxiliary)
- Checkpoints include "model_version": "v10"
"""
...
# ── V10 config extension ──
Config.v10_pose_aux_weight = 0.05
...
def build_v10_model(seq_len=8, img_size=224, hidden_dim=512, n_heads=8,
                     n_layers=4, ff_dim=2048, dropout=0.1, pretrained=True,
                     num_excavators=4):
    """Construct a V10 model with temporal mask mixer."""
...
def train_epoch(model, dataloader, optimizer, scaler, scheduler, criterion, config, epoch):
    ...
    for step, batch in enumerate(pbar):
        ...
        with autocast():
            # V10: return_aux=True for training-only pose auxiliary
...
def validate(model, dataloader, criterion, config):
    """V10 validation: normal 3-output path (no auxiliary)."""
    model.eval()
    total_loss = 0.0
    total_mae = np.zeros(4)
    ss_res = np.zeros(4)
...
def main():
    parser = argparse.ArgumentParser(description="Train ExcavatorVLA-YOLO V10")
    ...
    parser.add_argument("--pose_aux_weight", type=float, default=None,
    ...
    if args.pose_aux_weight is not None:
        config.v10_pose_aux_weight = args.pose_aux_weight

    config.device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Device: {config.device}")
    print(f"V10 pose_aux_weight: {config.v10_pose_aux_weight}")
